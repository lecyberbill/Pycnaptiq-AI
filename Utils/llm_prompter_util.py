...
def on_enhance_or_redo_button_click(current_text_in_box, original_prompt_for_cycle, cycle_is_active, llm_model_path, current_translations):
    """
    Handler pour le bouton d'amélioration (ou de ré-amélioration) du prompt.
    """
    if not current_text_in_box or not current_text_in_box.strip():
        gr.Warning(translate("prompt_vide_erreur", current_translations), 3.0)
        return (current_text_in_box, gr.update(), gr.update(), original_prompt_for_cycle, False, False, None)
    ...
    if llm_model_prompter is None or llm_tokenizer_prompter is None:
        ...
        success_load = init_llm_prompter(llm_model_path, current_translations)
        if not success_load:
    ...
    enhanced_prompt_candidate = generate_enhanced_prompt(prompt_to_enhance_this_time, llm_model_path, translations=current_translations)
    ...
    if enhanced_prompt_candidate and enhanced_prompt_candidate.strip() and enhanced_prompt_candidate.strip().lower() != prompt_to_enhance_this_time.strip().lower():
        ...
        return (current_text_in_box,
                ...
                gr.update(value=cycle_is_active), 
                gr.update(value=current_text_in_box if cycle_is_active else None))

def on_validate_prompt_button_click(validated_prompt_text, current_translations):
    """
    Handler pour valider le prompt amélioré et clore le cycle.
    """
...
def handle_text_input_change(text_value, last_ai_output_val, is_cycle_active_val, llm_model_path, current_translations):
    """
    Gère les changements de texte dans le prompt pour adapter l'état de l'interface LLM.
    """
    enhance_button_interactive = bool(text_value.strip())
    if not text_value:
        if llm_model_prompter is not None or llm_tokenizer_prompter is not None:
            unload_llm_prompter(current_translations)
        ...
        return (gr.update(value=translate("ameliorer_prompt_ia_btn", current_translations), interactive=enhance_button_interactive), 
                gr.update(visible=False), gr.update(value=""), gr.update(value=False), gr.update(value=False), gr.update(value=None))
    else:
        if is_cycle_active_val and text_value == last_ai_output_val:
            return (gr.update(value=translate("refaire_amelioration_btn", current_translations), interactive=True), 
                    gr.update(visible=True, interactive=True), gr.update(), gr.update(value=True), gr.update(value=True), gr.update())
        else:
            if is_cycle_active_val: 
                gr.Info(translate("prompt_modifie_reinitialisation_amelioration", current_translations), 2.0)
            return (gr.update(value=translate("ameliorer_prompt_ia_btn", current_translations), interactive=enhance_button_interactive), 
                    gr.update(visible=False), gr.update(value=text_value), gr.update(value=False), gr.update(value=False), gr.update(value=None))
