...
## 🏗️ New Modular Architecture (Version 2.1.0) 

Pycnaptiq-AI has been significantly refactored for better modularity and maintainability. The main logic is no longer contained within a single file.

- **`core/`**: Centralized backend logic (SDXL generation, Inpainting, Model Loaders, Config handling).
- **`Utils/`**: Shared utilities, Preset handlers, Sampler definitions, and Memory management.
- **`Pycnaptiq-AI.py`**: Now focused primarily on UI layout and tab declarations.
- **`modules/`**: Dynamic module system for adding new features easily.

This architecture allows for faster feature updates and a more stable user experience.
