    ...
    **kwargs # Accepter les arguments supplémentaires
    ...
    def run_pipeline_thread():
        try:
            ...
            # --- AJOUT: Passer les arguments ControlNet ---
            if control_image is not None:
                # Si c'est un pipeline ControlNet standard (Text2Img), l'argument est 'image'
                # StableDiffusionXLControlNetPipeline utilise 'image' pour l'image de contrôle
                from diffusers import StableDiffusionXLControlNetPipeline
                if isinstance(pipe, StableDiffusionXLControlNetPipeline):
                     pipeline_args["image"] = control_image
                else:
                    # Pour d'autres types (ex: Inpaint), on garde 'control_image'
                     pipeline_args["control_image"] = control_image 
                pipeline_args["controlnet_conditioning_scale"] = float(controlnet_conditioning_scale)
            
            # --- AJOUT: Passer l'image IP-Adapter ---
            if ip_adapter_image is not None:
                pipeline_args["ip_adapter_image"] = ip_adapter_image
                # Note: ip_adapter_scale est géré via pipe.set_ip_adapter_scale avant l'exécution
