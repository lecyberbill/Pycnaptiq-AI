...
from Utils import gradio_components as ggr  # AJOUT
from Utils import llm_prompter_util # AJOUT
...
class CogView3PlusModule:
    def __init__(self, global_translations, model_manager_instance: ModelManager, gestionnaire_instance, global_config=None):
        ...
        # --- AJOUT: Configuration pour l'amélioration du prompt ---
        self.llm_prompter_model_path = self.global_config.get("LLM_PROMPTER_MODEL_PATH", "Qwen/Qwen2.5-0.5B-Instruct")
    ...
    def create_tab(self, module_translations):
        ...
        with gr.Tab(translate("cogview3plus_tab_name", self.module_translations)) as tab:
            ...
            cog_original_user_prompt_state = gr.State(value="")
            cog_current_prompt_is_enhanced_state = gr.State(value=False)
            cog_enhancement_cycle_active_state = gr.State(value=False)
            cog_last_ai_enhanced_output_state = gr.State(value=None)
            # --- FIN AJOUT ---
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1, variant="panel"):
                    (self.cogview3plus_prompt, 
                     self.cogview3plus_enhance_button, 
                     self.cogview3plus_validate_button, 
                     self.cogview3plus_traduire_checkbox, 
                     self.cogview3plus_style_dropdown) = ggr.create_prompt_interface(self, self.module_translations, prefix="cogview3plus_")

                    (self.cogview3plus_resolution_dropdown, 
                     self.cogview3plus_steps_slider, 
                     self.cogview3plus_guidance_scale_slider, 
                     self.cogview3plus_seed_input, 
                     self.cogview3plus_num_images_slider) = ggr.create_generation_settings(
                         self, self.module_translations, 
                         prefix="cogview3plus_",
                         allowed_resolutions=self.allowed_resolutions, 
                         default_steps=50, max_steps=100
                     )
                     
                    # Surcharge info résolution spécifique à CogView3Plus
                    self.cogview3plus_resolution_dropdown.info = translate("resolution_info_cogview3plus", self.module_translations)
                    
                    self.cogview3plus_use_image_prompt_checkbox = gr.Checkbox(
                        label=translate("generer_prompt_image", self.global_translations),
                        value=False
                    )
                    self.cogview3plus_image_input_for_prompt = gr.Image(
                ...
                with gr.Column(scale=1, variant="panel"):
                    (self.cogview3plus_message_chargement, 
                     self.cogview3plus_result_output, 
                     self.cogview3plus_bouton_gen, 
                     self.cogview3plus_bouton_stop, 
                     self.cogview3plus_progress_html) = ggr.create_output_interface(self.module_translations, prefix="cogview3plus_")
            ...
            cog_gen_inputs = [
                ...
                self.cogview3plus_guidance_scale_slider,
                self.cogview3plus_seed_input,
                cog_original_user_prompt_state,
                cog_current_prompt_is_enhanced_state,
                cog_enhancement_cycle_active_state,
            ]

            self.cogview3plus_bouton_gen.click(
                fn=self.cogview3plus_gen,
                inputs=cog_gen_inputs,
                outputs=[
                    self.cogview3plus_result_output,
            ...
            self.cogview3plus_enhance_button.click(
                fn=self.on_cog_enhance_or_redo_button_click,
                inputs=[self.cogview3plus_prompt, cog_original_user_prompt_state, cog_enhancement_cycle_active_state, gr.State(self.llm_prompter_model_path), gr.State(self.module_translations)],
                outputs=[self.cogview3plus_prompt, self.cogview3plus_enhance_button, self.cogview3plus_validate_button, cog_original_user_prompt_state, cog_current_prompt_is_enhanced_state, cog_enhancement_cycle_active_state, cog_last_ai_enhanced_output_state]
            )
            self.cogview3plus_validate_button.click(
                fn=self.on_cog_validate_prompt_button_click,
                inputs=[self.cogview3plus_prompt, gr.State(self.module_translations)],
                outputs=[self.cogview3plus_enhance_button, self.cogview3plus_validate_button, cog_original_user_prompt_state, cog_current_prompt_is_enhanced_state, cog_enhancement_cycle_active_state, cog_last_ai_enhanced_output_state]
            )
            self.cogview3plus_prompt.input(
                fn=self.handle_cog_text_input_change,
                inputs=[self.cogview3plus_prompt, cog_last_ai_enhanced_output_state, cog_enhancement_cycle_active_state, gr.State(self.llm_prompter_model_path), gr.State(self.module_translations)],
                outputs=[
                    self.cogview3plus_enhance_button,
                    self.cogview3plus_validate_button,
                    cog_original_user_prompt_state,
                    cog_current_prompt_is_enhanced_state,
                    cog_enhancement_cycle_active_state,
                    cog_last_ai_enhanced_output_state
                ]
            )
            self.cogview3plus_prompt.submit(
                fn=self.handle_cog_text_input_change,
                inputs=[self.cogview3plus_prompt, cog_last_ai_enhanced_output_state, cog_enhancement_cycle_active_state, gr.State(self.llm_prompter_model_path), gr.State(self.module_translations)],
                outputs=[
                    self.cogview3plus_enhance_button,
    ...
    def on_cog_enhance_or_redo_button_click(self, current_prompt, original_user_prompt, cycle_active, model_path, translations):
    ...
    def on_cog_validate_prompt_button_click(self, current_prompt, translations):
        return llm_prompter_util.on_validate_prompt_button_click(current_prompt, translations)

    def handle_cog_text_input_change(self, current_prompt, last_ai_output, cycle_active, model_path, translations):
    ...
    def cogview3plus_gen(
        ...
        # On utilise le seed fourni s'il est >= 0, sinon on en génère un aléatoire
        base_seed = int(seed_input) if seed_input >= 0 else random.randint(0, 2**32 - 1)
