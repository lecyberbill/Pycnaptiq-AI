...
module_json_path = os.path.join(os.path.dirname(__file__), "photo_editing_mod.json")
...
def initialize(global_translations, global_pipe=None, global_combel=None, global_config=None):
    """Initialise le module photo editing."""
    print(txt_color("[OK] ", "ok"), module_data["name"])
    return PhotoEditing(global_translations, global_pipe, global_combel,  global_config=None) 


class PhotoEditing:
    def __init__(self, global_translations, global_pipe=None, global_combel=None, global_config=None):
        self.image_editor = None
        self.global_translations = global_translations
        self.global_pipe = None
        self.global_combel = None
        self.global_config = None
        

    def create_tab(self, module_translations):
        """Crée l'onglet Gradio pour la retouche d'image."""
        with gr.Tab(translate("retouche_image", module_translations)) as tab:
            with gr.Row(visible=True) as edit_controls:
                with gr.Column():
                    edit_image_input = gr.Image(label=translate("selectionner_une_image", module_translations), type="numpy")
                with gr.Column():
                    with gr.Row(visible=True) as edit_controls:
                        with gr.Column():
                            with gr.Accordion(translate("Transformations", module_translations), open=False):
                                rotation_angle = gr.Slider(0, 360, 0, step=90, label=translate("angle_de_rotation_90", module_translations))
                                mirror_type = gr.Dropdown(choices=["aucun", "horizontal", "vertical"], value="aucun", label=translate("type_de_miroir", module_translations))
                                special_filter = gr.Dropdown(choices=["aucun", "sepia", "contour", "negative", "posterize", "solarize", "emboss", "pixelize", "vignette", "mosaic"], value="aucun", label=translate("filtre_special", module_translations))    
                            with gr.Accordion(translate("Retouche_simples", module_translations), open=False):
                                contrast = gr.Slider(0.5, 2.0, 1.0, step=0.1, label=translate("contraste", module_translations))
                                saturation = gr.Slider(0.5, 2.0, 1.0, step=0.1, label=translate("saturation", module_translations))
                                color_boost = gr.Slider(0.5, 2.0, 1.0, step=0.1, label=translate("intensite_des_couleurs", module_translations))
                                blur_radius = gr.Slider(0, 10, 0, step=1, label=translate("rayon_de_flou", module_translations))
                                sharpness_factor = gr.Slider(0, 5, 1, step=0.1, label=translate("facteur_de_nettete", module_translations))
                                grayscale = gr.Checkbox(label=translate("noir_et_blanc", module_translations))
                            with gr.Accordion(translate("vibrance", module_translations), open=False):
                                vibrance = gr.Slider(0, 2, 0, step=0.1, label=translate("vibrance", module_translations))
                                hue_angle = gr.Slider(-180, 180, 0, step=1, label=translate("teinte", module_translations))
                            with gr.Accordion(translate("courbes", module_translations), open=False):
                                with gr.Row():
                                    point1 = gr.Slider(0, 1, 0, step=0.01, label=translate("point_courbe_1", module_translations))
                                    point2 = gr.Slider(0, 1, 0, step=0.01, label=translate("point_courbe_2", module_translations))
                                    point3 = gr.Slider(0, 1, 1, step=0.01, label=translate("point_courbe_3", module_translations))
                            with gr.Accordion(translate("nettete_adaptative", module_translations), open=False):
                                unsharp_radius = gr.Slider(0, 10, 0, step=1, label=translate("rayon_flou", module_translations))
                                unsharp_percent = gr.Slider(0, 300, 100, step=10, label=translate("pourcentage_nettete", module_translations))
                                unsharp_threshold = gr.Slider(0, 20, 0, step=1, label=translate("seuil_difference", module_translations))
                            with gr.Accordion(translate("bruit", module_translations), open=False):
                                noise_amount = gr.Slider(0, 1, 0, step=0.01, label=translate("quantite_bruit", module_translations))
                            with gr.Accordion(translate("degrade_couleur", module_translations), open=False):
                                gradient_active = gr.Checkbox(label=translate("activer_degrade", module_translations), value=False)
                                gradient_angle = gr.Slider(0, 360, 0, step=1, label=translate("angle_degrade", module_translations))
                                gradient_color1 = gr.ColorPicker(value="#FF0000", label=translate("couleur1", module_translations))
                                gradient_color2 = gr.ColorPicker(value="#0000FF", label=translate("couleur2", module_translations))
                            with gr.Accordion(translate("decalage_couleur", module_translations), open=False):
                                color_shift_r = gr.Slider(-255, 255, 0, step=1, label=translate("decalage_rouge", module_translations))
                                color_shift_g = gr.Slider(-255, 255, 0, step=1, label=translate("decalage_vert", module_translations))
                                color_shift_b = gr.Slider(-255, 255, 0, step=1, label=translate("decalage_bleu", module_translations))
                with gr.Column():
                    edit_image_output = gr.Image(label=translate("apercu_des_modifications", module_translations), type="numpy")
            ...
            for inp in inputs:
                inp.change(self.apply_filters_to_image, inputs=inputs, outputs=edit_image_output)
        return tab

    def apply_filters_to_image(self, image, contrast, saturation, color_boost, grayscale, blur_radius, sharpness_factor, rotation_angle, mirror_type, special_filter, vibrance, hue_angle, point1, point2, point3, unsharp_radius, unsharp_percent, unsharp_threshold, noise_amount, gradient_color1, gradient_color2, gradient_active, color_shift_r, color_shift_g, color_shift_b, gradient_angle):
        ...
        try:
            ...
            print(txt_color("[ERREUR] ", "erreur"), f"{translate('erreur_image_invalide', module_translations)} : {e}")
            raise gr.Error(translate("erreur_image_invalide", module_translations) + f" : {e}", 4.0)
            return None
