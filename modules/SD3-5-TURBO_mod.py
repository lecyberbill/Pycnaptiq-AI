...
class SD3_5_TURBO_Module:
        ...
        with gr.Tab(translate("sd3_5_turbo_tab_name", self.module_translations)) as tab:
            ...
            with gr.Row():
                ...
                with gr.Column(scale=1, variant="panel"):
                    (self.sd3_prompt, 
                     self.sd3_enhance_or_redo_button, 
                     self.sd3_validate_prompt_button, 
                     self.sd3_traduire_checkbox, 
                     self.sd3_style_dropdown) = ggr.create_prompt_interface(self, self.module_translations, prefix="sd3_5_turbo_")

                    (self.sd3_resolution_dropdown, 
                     self.sd3_steps_slider, 
                     self.sd3_guidance_scale_slider, 
                     self.sd3_seed_input, 
                     self.sd3_num_images_slider) = ggr.create_generation_settings(
                         self, self.module_translations, 
                         prefix="sd3_5_turbo_",
                         allowed_resolutions=self.allowed_resolutions, 
                         default_steps=4, max_steps=50
                     )
                ...
                with gr.Column(scale=1, variant="panel"):
                    (self.sd3_message_chargement, 
                     self.sd3_result_output, 
                     self.sd3_bouton_gen, 
                     self.sd3_bouton_stop, 
                     self.sd3_progress_html) = ggr.create_output_interface(self.module_translations, prefix="sd3_5_turbo_")
                    
                    gr.Markdown("---")
                    
                    with gr.Group():
                        gr.Markdown(f"### {translate('sd3_5_turbo_model_select_label', self.module_translations)}")
                        with gr.Row():
                            ...
                            self.sd3_refresh_models_button = gr.Button(
                                value="🔄", min_width=60, scale=1, elem_id="sd3_refresh_models_button"
                            )
                    ...
                    (self.sd3_lora_checks, 
                     ...
                     self.sd3_lora_message, 
                     self.sd3_refresh_lora_button) = ggr.create_lora_interface(self, self.module_translations)
    ...
    def on_sd3_enhance_or_redo_button_click(self, current_prompt, original_user_prompt, cycle_active, model_path, translations):
    ...
    def on_sd3_validate_prompt_button_click(self, current_prompt, translations):
        return llm_prompter_util.on_validate_prompt_button_click(current_prompt, translations)

    def handle_sd3_text_input_change(self, current_prompt, last_ai_output, cycle_active, model_path, translations):
