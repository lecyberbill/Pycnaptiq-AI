...
def initialize(global_translations, global_pipe = None, global_compel=None, global_config=None):
    ...
    return RemBGModule(global_translations, global_pipe, global_compel, global_config)
...
class RemBGModule:
    ...
    def create_tab(self, module_translations):
        ...
        with gr.Tab(module_data["tab_name"]) as tab:
            gr.Markdown(f"## {translate('rembr_sr_title', module_translations)}")
            ...
            remove_bg_button = gr.Button(translate("remove_bg_button", module_translations))  # Button below
            ...
            def removeBG(image):
                ...
                chemin_image = os.path.join(save_dir, f"aura_sr_{module_translations.get('rem_bg_image', 'rem_bg_image')}_{date_str}_{heure_str}.png")
                enregistrer_image(upscaled_image, chemin_image, self.global_translations, "PNG")
            ...
            remove_bg_button.click(fn=removeBG, inputs=image_input, outputs=image_output, api_name="rem_bg_image")

        return tab
    
    def upscaleImage_BY_AuraSR(self, image):
        """
        Remove BG with remmbg.
        Args:
            image (PIL.Image): The image to remove backgroud.
        ...
        print(txt_color("[INFO] ", "info"), translate("remove_bg_with_rembg", self.global_translations))
        gr.Info(translate("remove_bg_with_rembg", self.global_translations), 3.0)
